...
class OpenSearchClientTest {
  ...
  void testPlainGet() {
    var expectedUri =
        OPENSEARCH_ENDPOINT_PROTOCOL
            + PROTOCOL_DELIMITER
            + OPENSEARCH_ENDPOINT_ADDRESS
            + "/sondre-test";
  ...
  void testBasicQueryParameters() {
    ...
    OpenSearchResponse response =
        openSearchClient.sendRequest(POST, "sondre-test/_search?size=1", null);
    assertEquals(200, response.getStatus());
  }
  ...
  void testAdvancedQueryParameters() {
    ...
    OpenSearchResponse response =
        openSearchClient.sendRequest(POST, "sondre-test/_search?q=age:>33", null);
    assertEquals(200, response.getStatus());
  }
}
