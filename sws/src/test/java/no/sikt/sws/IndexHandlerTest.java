...
class IndexHandlerTest {
  ...
  @InjectMocks private final IndexHandler handler = new IndexHandler();
  ...
  void shouldGiveResponse() throws IOException {
    ...
    when(openSearchClient.sendRequest(
            GET, TEST_PREFIX_SONDRE + "-" + TEST_INDEX_1 + MAPPING_PATH, null))
  ...
  void shouldPassQueryParams() throws IOException {
    ...
    final OpenSearchResponse mockResponse =
        new OpenSearchResponse(
            200, "{\"hits\":{\"total\":{\"value\":0,\"relation\":\"eq\"},\"hits\":[]}}");

    when(openSearchClient.sendRequest(GET, "workspace-sondre-" + uri, null))
        .thenReturn(mockResponse);
  ...
  void shouldReturnShardsFailures() throws IOException {
    ...
    var responseStr =
        "{\"took\":47354,\"timed_out\":false,\"_shards\":{\"total\":5,\"successful\":4,"
            + "\"skipped\":0,\"failed\":1,"
            + "\"failures\":[{\"shard\":3,\"index\":\""
            + TEST_PREFIX_SONDRE
            + "-"
            + TEST_INDEX_1
            + "\",\"node\":\"KtsKT2hORLCAKelYrRrX-Q\",\"reason\":"
            + "{\"type\":\"rejected_execution_exception\",\"reason\":\"cancelled task with reason:"
            + " heap usage exceeded [283.4mb >= 69.2mb]\"}}]}}";
    final OpenSearchResponse mockResponse = new OpenSearchResponse(500, responseStr);

    when(openSearchClient.sendRequest(
            GET, TEST_PREFIX_SONDRE + "-" + TEST_INDEX_1 + "/_search", null))
        .thenReturn(mockResponse);
  ...
  void shouldPassSearchScrollDirectlyToOpensearch() throws IOException {
    ...
    when(openSearchClient.sendRequest(GET, SEARCH_SCROLL, null)).thenReturn(mockResponse);
  ...
  Stream<DynamicTest> testRequestWithQueryParameters() {
    ...
    return DynamicTest.stream(
        requestsWithParameters, CaseSws::getName, this::assertTestCaseWithRequestQueryParameters);
  }
  ...
  private void assertTestCaseWithRequestQueryParameters(CaseSws testCase) throws IOException {
    ...
    var response = GatewayResponse.fromOutputStream(output, String.class);
    var resultBody =
        SearchDto.fromResponse(response.getBody()).stripper(TEST_PREFIX_MOCKNAME).toJsonCompact();

    Assertions.assertEquals(
        stripWhitespaceNewLine(testCase.getResponseStripped()), stripWhitespaceNewLine(resultBody));
  ...
  void shouldHandleSearchRequestWithAggregation() throws IOException {
    ...
    when(openSearchClient.sendRequest(
            requestOpensearch.getMethod(), requestOpensearch.getUrl(), requestOpensearch.getBody()))
        .thenReturn(new OpenSearchResponse(200, testcase.getResponse()));
    ...
    var request =
        buildRequestWithBody(
            HttpMethod.GET, pathParams, requestGateway.getBody(), TEST_SCOPE_SONDRE);

    handler.handleRequest(request, output, CONTEXT);

    var response = GatewayResponse.fromOutputStream(output, String.class);
    var resultBody =
        SearchDto.fromResponse(response.getBody()).stripper(TEST_PREFIX_SONDRE).toJsonCompact();
  ...
  private Stream<CaseSws> getSearchRequestTestCasesStream() {
    return new CaseLoader.Builder().loadResource("proxy/requests-search.json").build();
  }
