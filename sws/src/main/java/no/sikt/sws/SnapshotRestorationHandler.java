...
public class SnapshotRestorationHandler extends ApiGatewayHandler<SnapshotToRestoreDto, String> {
  ...
  protected void validateRequest(
      SnapshotToRestoreDto snapshotToRestoreDto, RequestInfo requestInfo, Context context)
  ...
  protected String processInput(
      SnapshotToRestoreDto input, RequestInfo requestInfo, Context context)
      throws ApiGatewayException {
    LOGGER.info("retrieved name of snapshot: " + input.snapshotName);
    LOGGER.info("full call : " + SNAPSHOT_REPO_NAME + input.snapshotName + RESTORE_COMMAND);
    var snapshotRepoPathRequest = SNAPSHOT_REPO_NAME + input.snapshotName + RESTORE_COMMAND;
    try {
      var response =
          openSearchClient.sendRequest(HttpMethodName.POST, snapshotRepoPathRequest, null);
