...
public class CognitoHandler extends ApiGatewayHandler<CreateUserClientDto, CognitoCredentialsDto> {
  ...
  protected void validateRequest(
      CreateUserClientDto createUserClientDto, RequestInfo requestInfo, Context context)
      throws ApiGatewayException {
    // no op
  }

  @Override
  protected CognitoCredentialsDto processInput(
      CreateUserClientDto input, RequestInfo requestInfo, Context context) {
    ...
    if (!input.name.matches(ALLOWED_NAME_REGEX)) {
      throw new IllegalStateException(
          "Name contains illegal chars. Should only be letters and numbers");
    }
  ...
  private CognitoCredentialsDto getClientCredentials(String userPoolId, String clientName) {
    var listClientRequest =
        ListUserPoolClientsRequest.builder().maxResults(50).userPoolId(userPoolId).build();

    var listClientResponse = cognitoClient.listUserPoolClients(listClientRequest);
    var client =
        listClientResponse.userPoolClients().stream()
    ...
    var describeClientRequest =
        DescribeUserPoolClientRequest.builder()
            .userPoolId(userPoolId)
            .clientId(client.get().clientId())
            .build();
    var clientDescription =
        cognitoClient.describeUserPoolClient(describeClientRequest).userPoolClient();

    return new CognitoCredentialsDto(
        clientDescription.clientId(), clientDescription.clientSecret());
  }

  private void createAppClient(String userPoolId, String name, String appClientName) {

    var tokenValidityUnitTypes =
    ...
    var createUserPoolRequest =
        CreateUserPoolClientRequest.builder()
            ...
            .generateSecret(true)
            .allowedOAuthScopes(
                Lists.newArrayList(SCOPE_IDENTIFIER + "/workspace", SCOPE_IDENTIFIER + "/" + name))
  ...
  private ResourceServerType getResourceServer(String userPoolId) {
    var listResourceServersRequest =
        ListResourceServersRequest.builder().userPoolId(userPoolId).maxResults(50).build();
    var resources = cognitoClient.listResourceServers(listResourceServersRequest);

    var server =
        resources.resourceServers().stream()
            .filter(s -> BACKEND_SCOPE_RESOURCE_SERVER_NAME.equals(s.name()))
            .findFirst();
    if (server.isEmpty()) {
  ...
  private String getUserPoolId() {
    var userPool =
        cognitoClient.listUserPools(ListUserPoolsRequest.builder().build()).userPools().stream()
            .filter(up -> USER_POOL_NAME.equals(up.name()))
            .findFirst();
