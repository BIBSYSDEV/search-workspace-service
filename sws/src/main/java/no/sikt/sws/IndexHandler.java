...
public class IndexHandler extends ApiGatewayProxyHandler<String, String> {
  ...
  public static final String TOO_LARGE_RESULTS =
      "Search response too large. Try to narrow down the search or use " + "pagination";
  ...
  protected ProxyResponse<String> processProxyInput(
      String body, RequestInfo request, Context context) throws ApiGatewayException {
    ...
    try {
      ...
      var responseKind =
          OpenSearchResponseKind.fromString(httpMethod, commandKind, response.getBody());

      var responseBody =
          PrefixStripper.body(commandKind, responseKind, workspace, response.getBody());
  ...
  private static void assertThatLambdaCanHandleResponseSize(String responseBody)
  ...
  private static void validateResourceIdentifier(
      String resourceIdentifier, OpenSearchCommandKind commandKind) throws BadRequestException {

    if (commandKind.isNotValid()) {
      throw new BadRequestException(
          "Root operations and indices starting with '_' or containing anything but letters,"
              + " digits, '/', '-' or '_'are not allowed. Got: "
              + resourceIdentifier);
    }
  }
}
