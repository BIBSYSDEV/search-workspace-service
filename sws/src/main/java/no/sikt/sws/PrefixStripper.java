...
public class PrefixStripper {
  ...
  public static JsonNode node(JsonNode node, String workspacePrefix) {
    if (workspacePrefix == null) {
      throw new IllegalArgumentException(
          REQUIRED_PARAMETER_IS_NULL + ApplicationConstants.WORKSPACE_PREFIX);
    }
    if (node == null) {
      return null;
    }
    ...
    return string2JsonNode(node.toString().replaceAll(regex, replaceWithGroup1andGroup3));
  }
  ...
  public static String body(
    ...
    return switch (responseKind) {
      ...
      case ERROR -> ErrorDto.fromResponse(responseBody).stripper(workspacePrefix).toJsonCompact();
      case CONTENT ->
          switch (commandKind) {
            case ALIAS, MAPPING ->
                IndexDto.fromResponse(responseBody).stripper(workspacePrefix).toJsonCompact();
            case DOC -> DocDto.fromResponse(responseBody).stripper(workspacePrefix).toJsonCompact();
      ...
      case CONTENT_COLLECTION ->
          switch (commandKind) {
            ...
            case INDEX ->
                IndexesDto.fromResponse(responseBody).stripper(workspacePrefix).toJsonCompact();
            case SEARCH, SCROLL ->
                SearchDto.fromResponse(responseBody).stripper(workspacePrefix).toJsonCompact();
            default -> throw new IllegalStateException(UNEXPECTED_VALUE + commandKind.name());
          };
    };
  }
  ...
  private static void validateParameters(
      String workspacePrefix, String responseBody, OpenSearchCommandKind commandKind) {
